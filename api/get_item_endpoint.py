...
class GetItemEndpoint(BaseEndpoint):
    ...
    def action(self, item_id: int) -> Dict[str, Any]:
        ...
        validate_get_item_response(self.response_json)
        return self.response_json
    
    @allure.step("Получаем информацию о товаре с ID: {item_id}, ожидая ошибку с кодом {expected_code}")
    def action_expect_error(self, item_id: int, expected_code: int) -> Dict[str, Any]:
